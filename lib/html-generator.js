...
function renderMessage(msg) {
  const time = `<span class="timestamp">${formatTime(msg.timestamp)}</span>`;

  switch (msg.type) {
    case 'user':
      return `<div class="msg msg-user">${time}<div class="msg-content" data-markdown>${escapeHtml(msg.text)}</div></div>`;

    case 'assistant':
      return `<div class="msg msg-assistant">${time}<div class="msg-content" data-markdown>${escapeHtml(msg.text)}</div></div>`;

    case 'thinking':
      return `<div class="msg msg-thinking">${time}<span class="thinking-indicator">Thinking...</span></div>`;

    case 'tool_use': {
      const summary = toolUseSummary(msg);
      const resultHtml = renderToolResult(msg);
      return `<div class="msg msg-tool"><details><summary>${time}<span class="tool-summary">${summary}</span></summary><div class="tool-details">${renderToolInput(msg)}${resultHtml}</div></details></div>`;
    }

    case 'tool_result':
      // Unpaired tool result (shouldn't happen often)
      return `<div class="msg msg-tool"><div class="tool-output"><pre><code>${escapeHtml((msg.content || '').slice(0, 2000))}</code></pre></div></div>`;

    case 'system':
      if (msg.subtype === 'turn_duration') {
        return `<div class="msg msg-system"><span class="duration">Turn: ${formatDuration(msg.durationMs)}</span></div>`;
      }
      return `<div class="msg msg-system">${escapeHtml(msg.content || '')}</div>`;
...
function generate(parsed, options = {}) {
  ...
  // Render subagents
  const subagentHtml = Object.entries(parsed.subagents || {})
    .map(([id, msgs]) => renderSubagent(id, msgs))
    .join('\n');
